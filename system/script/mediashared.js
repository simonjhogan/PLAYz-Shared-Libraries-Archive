...
MediaShared.prototype.initialise = function()
{
	var self = this;
	this.plex = new PLEX();
	this.cache = "";
...
MediaShared.prototype.loadMenu = function(section, key)
{
	var i = 0;
	var self = this;
	...
	this.plex.getSharedSectionDetails(self.serverUrl, self.serverToken, key, function(xml) {
		//Set title
		$("#applicationWallpaper").css("background-image", "url(" + self.plex.getSharedTranscodedPath(self.serverUrl, self.serverToken, $(xml).find("MediaContainer:first").attr("art"), 1280, 720) + ")");
		...
		$(xml).find("Directory[search!='1'][secondary!='1']").each(function(index, item) {
			html = "<li><a href data-key-index=\"" + i++ + "\" data-action=\"view\" data-server-url=\"" + self.serverUrl + "\" data-server-token=\"" + self.serverToken + "\" data-section=\"" + self.section + "\" data-key=\"" + self.key + "\" data-filter=\"" + $(this).attr("key") + "\">" + $(this).attr("title").replace("By ", "") + "</a></li>";
		...
		$(xml).find("Directory[secondary='1']").each(function(index, item) {
			html = "<li><a href data-key-index=\"" + i++ + "\" data-action=\"view\" data-server-url=\"" + self.serverUrl + "\" data-server-token=\"" + self.serverToken + "\" data-section=\"" + self.section + "\" data-key=\"" + self.key + "\" data-filter=\"" + $(this).attr("key") + "\">" + $(this).attr("title").replace("By ", "") + "</a></li>";			$("#menuFilterContent ul").append(html);
		});
...
MediaShared.prototype.view = function(section, key, filter, filterKey)	
{
	this.rowCount = 0;
	...
	self.plex.getSharedSectionMedia(self.serverUrl, self.serverToken, key, filter, filterKey, function(xml) {
		...
		$(xml).find("Directory,Video,Photo,Artist,Track").each(function(index, item) {
			if (localStorage.getItem(self.PLEX_VIEW_MODE) == "list") {
				html = self.plex.getSharedListHtml(self.serverUrl, self.serverToken, index, $(this).attr("title"), self.section, $(this).attr("type"), $(this).attr("key"), 
				...
				html = self.plex.getSharedThumbHtml(self.serverUrl, self.serverToken, index, $(this).attr("title"), self.section, $(this).attr("type"), $(this).attr("key"), 
		...
		$("#mediaViewContent a").focus(function(event) {
			...
			if (localStorage.getItem(self.PLEX_OPTIONS_PREFIX + "hoverBackdrop") == "1") {
				$("#applicationWallpaper").css("background-image", "url(" + self.plex.getSharedTranscodedPath(self.serverUrl, self.serverToken, $(this).data("art"), 1280, 720) + ")");
			}
		...
		$("#mediaViewContent a").click(function(event) {
			...
			if ($(this).is("[data-filter]")) {
				url = "./mediashared.html?action=view&section=" + self.section + "&key=" + $(this).data("sectionKey") + "&filter=" + self.filter + "&filterkey=" + encodeURIComponent($(this).data("key")) + "&servername=" + self.serverName + "&serverurl=" + self.serverUrl + "&servertoken=" + self.serverToken;
			} else {
				url = "./itemshared.html?action=preview&section=" + self.section + "&sectionKey=" + $(this).data("sectionKey") + "&key=" + encodeURIComponent($(this).data("key")) + "&servername=" + self.serverName + "&serverurl=" + self.serverUrl + "&servertoken=" + self.serverToken;
			}
			$(this).attr("href", url);
			location.href = url;
		});
...
MediaShared.prototype.toggleMenu = function()
{
	if (this.menuFlag) {
		this.hideMenu();
	} else {
...
MediaShared.prototype.showMenu = function()
{
...
MediaShared.prototype.hideMenu = function()
{
...
MediaShared.prototype.showLoader = function(message)
{
	$("#message").text(message);
	$("#loader").show();
};

MediaShared.prototype.hideLoader = function()
{
	$("#loader").hide();
};

MediaShared.prototype.getRowCount = function(query) {
    var row = 0;
    $(query).each(function() {
...
MediaShared.prototype.setDebug = function()
{
	var self = this;
	var device = document.getElementById("device");
