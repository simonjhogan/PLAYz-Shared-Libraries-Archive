...
PLEX.prototype.getSharedServerPort = function(serverUrl) {
	return serverUrl.substr(serverUrl.lastIndexOf(":")+1);
};

PLEX.prototype.getMyPlexAccount = function() {
	return $.ajax({
		type: "GET",
		url: this.getServerUrl() + "/myplex/account",
		async: false
	}).responseXML ;
};

PLEX.prototype.getXMLSharedServers = function(token) {
	return $.ajax({
		type: "GET",
		url: "https://my.plexapp.com/pms/servers?auth_token=" + token,
		async: false
	}).responseXML ;
};

PLEX.prototype.getSharedServers = function() {
	var toReturn = [];	
	var account = this.getMyPlexAccount();	
	var servers = this.getXMLSharedServers($(account).find("MyPlex").attr("authToken"));
	
	$(servers).find("Server").each(function(index, item) {
		var toAdd = {
			accessToken: $(this).attr("accessToken"),
			name: $(this).attr("name"),
			address: $(this).attr("address"),
			port: $(this).attr("port"),
			scheme: $(this).attr("scheme"),
			sourceTitle: $(this).attr("sourceTitle"),
			machineIdentifier: $(this).attr("machineIdentifier"),
			owned: $(this).attr("owned")
		};
...
PLEX.prototype.checkSharedLibraryServerExists = function(serverUrl, token, callback, failCallback) {
	$.ajax({
		type: "GET",
		url: serverUrl,
		headers: {"X-Plex-Product": this.X_Plex_Product,
			"X-Plex-Device": this.X_Plex_Device,
			"X-Plex-Platform": this.X_Plex_Platform,
			"X-Plex-Platform-Version": this.X_Plex_Platform_Version,
			"X-Plex-Version": this.X_Plex_Version,
...
PLEX.prototype.getSharedSections = function(serverUrl, token, callback) {
	$.ajax({
		type: "GET",
...
PLEX.prototype.getSharedTranscodedPath = function(ServerUrl, ServerToken, path, width, height) {
	return ServerUrl + "/photo/:/transcode?url=" + encodeURIComponent("http://localhost:" + "32400" + path) + "&width=" + width + "&height=" + height + "&X-Plex-Token=" + ServerToken;
};

PLEX.prototype.getSharedTranscodedMediaFlagPath = function(ServerUrl, ServerToken, flag, value, width, height) {
	return ServerUrl + "/photo/:/transcode?url=" + encodeURIComponent("http://localhost:" + "32400" + "/system/bundle/media/flags/" + flag + "/" + value + "?t=" + new Date().getTime()) + "&width=" + width + "&height=" + height + "&X-Plex-Token=" + ServerToken;
};

PLEX.prototype.getSharedRecentlyAdded = function(serverUrl, token, key, callback) {
	$.ajax({
		type: "GET",
		url: serverUrl + "/library/sections/" + key + "/recentlyAdded?X-Plex-Container-Start=0&X-Plex-Container-Size=25&X-Plex-Access-Time=" + this.time,
		headers: {"X-Plex-Product": this.X_Plex_Product,
			"X-Plex-Device": this.X_Plex_Device,
			"X-Plex-Platform": this.X_Plex_Platform,
			"X-Plex-Platform-Version": this.X_Plex_Platform_Version,
...
PLEX.prototype.getSharedOnDeck = function(serverUrl, token, key, callback) {
	$.ajax({
		type: "GET",
		url: serverUrl + "/library/onDeck?X-Plex-Container-Start=0&X-Plex-Container-Size=25&X-Plex-Access-Time=" + this.time,
		headers: {"X-Plex-Product": this.X_Plex_Product,
			"X-Plex-Device": this.X_Plex_Device,
			"X-Plex-Platform": this.X_Plex_Platform,
			"X-Plex-Platform-Version": this.X_Plex_Platform_Version,
...
PLEX.prototype.getSharedMediaItems = function(serverUrl, token, section, key, callback) {
	if (section == "ondeck") {
		this.getSharedOnDeck(serverUrl, token, key, callback);
	} else {
		this.getSharedRecentlyAdded(serverUrl, token, key, callback);
	}
};

PLEX.prototype.getSharedThumbHtml = function(serverUrl, serverToken, index, title, sectionType, mediaType, key, metadata) {
	var html = "";
	if (sectionType && sectionType.indexOf("home") > -1) {
		mediaType = "home";
	}
	
	var serverPort = this.getSharedServerPort(serverUrl);
		
	switch(mediaType) {	
		case "movie":
			html = "<li class=\"media " + mediaType + "\"><a data-key-index=\"" + index + "\" data-server-url=\"" + serverUrl + "\" data-server-token=\"" + serverToken + "\" data-title=\"" + title + "\" data-key=\"" + key + "\" data-section-key=\"" + metadata.sectionKey + "\" data-section-type=\"" + sectionType + "\" data-media-type=\"" + mediaType + "\" data-art=\"" + metadata.art + "\" data-media=\"" + metadata.media + "\" href>";
			html += "<div class=\"thumb\" data-original=\"" + this.getSharedTranscodedPath(serverUrl, serverToken, metadata.thumb, 128, 190) + "\"></div>";	
			if (localStorage.getItem(this.PLEX_OPTIONS_PREFIX + "titleOverlay") != "1"){html += "<div class=\"subtitle alt\">" + title + "</div>";};
			if (localStorage.getItem(this.PLEX_OPTIONS_PREFIX + "watchedIcons") != "1") {
				html += "<div class=\"watchedStatus\">" + this.getWatchedIconHtml(metadata.lastViewedAt, metadata.viewOffset, metadata.viewCount, metadata.duration) + "</div>";
			}
		...
		case "photo":	
			if (metadata.media) {
				...
				html = "<li class=\"media " + mediaType + "\"><a data-key-index=\"" + index + "\" data-server-url=\"" + serverUrl + "\" data-server-token=\"" + serverToken + "\" data-title=\"" + title + "\" data-key=\"" + key + "\" data-section-key=\"" + metadata.sectionKey + "\" data-section-key=\"" + metadata.sectionKey + "\" data-section-type=\"" + sectionType + "\" data-media-type=\"folder\" data-art=\"" + metadata.art + "\" data-media=\"" + metadata.media + "\" data-filter=\"" + metadata.filter + "\" href>";
			}
			html += "<div class=\"thumb\" data-original=\"" + this.getSharedTranscodedPath(serverUrl, serverToken, metadata.thumb, 300, 190) + "\"></div>";	
			html += "<div class=\"subtitle alt\">" + title + "</div>";
			html += "</a></li>";			
			break;
		...
		case "home":	
			html = "<li class=\"media " + mediaType + "\"><a data-key-index=\"" + index + "\" data-server-url=\"" + serverUrl + "\" data-server-token=\"" + serverToken + "\" data-title=\"" + title + "\" data-key=\"" + key + "\" data-section-key=\"" + metadata.sectionKey + "\" data-section-type=\"" + sectionType + "\" data-media-type=\"" + mediaType + "\" data-art=\"" + metadata.art + "\" data-media=\"" + metadata.media + "\" href>";
			html += "<div class=\"thumb\" data-original=\"" + this.getSharedTranscodedPath(serverUrl, serverToken, metadata.thumb, 190, 190) + "\"></div>";	
			html += "<div class=\"subtitle alt\">" + title + "</div>";				
			html += "</a></li>";	
			break;
		...
		case "episode":		
			html = "<li class=\"media " + mediaType + "\"><a data-key-index=\"" + index + "\" data-server-url=\"" + serverUrl + "\" data-server-token=\"" + serverToken + "\" data-title=\"" + title + "\" data-key=\"" + key + "\" data-section-key=\"" + metadata.sectionKey + "\" data-section-type=\"" + sectionType + "\" data-media-type=\"" + mediaType + "\" data-art=\"" + metadata.art + "\" data-media=\"" + metadata.media + "\" href>";
			if (sectionType && sectionType.indexOf("recent") > -1) {
				html += "<div class=\"thumb\" data-original=\"" + this.getSharedTranscodedPath(serverUrl, serverToken, metadata.grandparentThumb, 300, 190) + "\"></div>";
			} else {
				html += "<div class=\"thumb\" data-original=\"" + this.getSharedTranscodedPath(serverUrl, serverToken, metadata.thumb, 300, 190) + "\"></div>";
			}
			if (localStorage.getItem(this.PLEX_OPTIONS_PREFIX + "titleOverlay") != "1"){if (metadata.series) {html += "<div class=\"title\">" + metadata.series + "</div>";};};
		...
		case "show":			
			html = "<li class=\"media " + mediaType + "\"><a data-key-index=\"" + index + "\" data-server-url=\"" + serverUrl + "\" data-server-token=\"" + serverToken + "\" data-title=\"" + title + "\" data-key=\"" + key + "\" data-section-key=\"" + metadata.sectionKey + "\" data-section-type=\"" + sectionType + "\" data-media-type=\"" + mediaType + "\" data-art=\"" + metadata.art + "\" href>";
			html += "<div class=\"thumb\" data-original=\"" + this.getSharedTranscodedPath(serverUrl, serverToken, metadata.thumb, 128, 190) + "\"></div>";
			if (localStorage.getItem(this.PLEX_OPTIONS_PREFIX + "titleOverlay") == "0") {html += "<div class=\"subtitle alt\">" + title + "</div>";};	
			if (localStorage.getItem(this.PLEX_OPTIONS_PREFIX + "watchedIcons") != "1") {
				html += "<div class=\"watchedStatus watchedCount\">" + (Number(metadata.leafCount) - Number(metadata.viewedLeafCount)) + "</div>";	
		...
		case "album":				
			html = "<li class=\"media " + mediaType + "\"><a data-key-index=\"" + index + "\" data-server-url=\"" + serverUrl + "\" data-server-token=\"" + serverToken + "\" data-title=\"" + title + "\" data-key=\"" + key + "\" data-section-key=\"" + metadata.sectionKey + "\" data-parent-key=\"" + metadata.parentKey + "\" data-section-type=\"" + sectionType + "\" data-media-type=\"" + mediaType + "\" data-art=\"" + metadata.art + "\" href>";
			html += "<div class=\"thumb\" data-original=\"" + this.getSharedTranscodedPath(serverUrl, serverToken, metadata.thumb, 190, 190) + "\"></div>";
			if (localStorage.getItem(this.PLEX_OPTIONS_PREFIX + "titleOverlay") != "1"){if (metadata.artist) {html += "<div class=\"title\">" + metadata.artist + "</div>";};};
			html += "<div class=\"subtitle alt\">" + title + "</div>";			
			html += "</a></li>";	
		...
		case "track":				
			html = "<li class=\"" + mediaType + "\"><a data-key-index=\"" + index + "\" data-server-url=\"" + serverUrl + "\" data-server-token=\"" + serverToken + "\" data-title=\"" + title + "\" data-key=\"" + key + "\" data-section-key=\"" + metadata.sectionKey + "\" data-section-type=\"" + sectionType + "\" data-media-type=\"" + mediaType + "\" data-media=\"" + metadata.media + "\" data-duration=\"" + metadata.duration + "\" href>";
			html += "<div class=\"track-title\"><i class=\"glyphicon\"></i>" + metadata.index + ". " + title + "</div>";			
			html += "</a></li>";	
			break;
		...
		default:				
			html = "<li class=\"media " + metadata.filter + "\"><a data-key-index=\"" + index + "\" data-server-url=\"" + serverUrl + "\" data-server-token=\"" + serverToken + "\" data-title=\"" + title + "\"";
			html += " data-key=\"" + key + "\" data-section-type=\"" + sectionType + "\" data-section-key=\"" + metadata.sectionKey + "\" data-media-type=\"folder\" data-filter=\"" + metadata.filter + "\" href>";
			html += "<div class=\"thumb\" data-original=\"" + this.getSharedTranscodedPath(serverUrl, serverToken, "/web/img/posters/folder.png", 128, 190) + "\"></div>";
			html += "<div class=\"subtitle\">" + title + "</div>";			
			html += "</a></li>";	
			break;
...
PLEX.prototype.getSharedListHtml = function(serverUrl, serverToken, index, title, sectionType, mediaType, key, metadata) {
	var html = "";
	if (sectionType && sectionType.indexOf("home") > -1) {
		mediaType = "home";
	}
	...
	switch(mediaType) {	
		...
		case "artist":
		case "album":				
			html = "<li class=\"list " + mediaType + "\"><a data-key-index=\"" + index + "\" data-server-url=\"" + serverUrl + "\" data-server-token=\"" + serverToken + "\" data-title=\"" + title + "\" data-key=\"" + key + "\" data-section-key=\"" + metadata.sectionKey + "\" data-parent-key=\"" + metadata.parentKey + "\" data-section-type=\"" + sectionType + "\" data-media-type=\"" + mediaType + "\" data-art=\"" + metadata.art + "\" href>";
			html += "<span class=\"listTitle\">" + title + "</div>";
			if (metadata.year) {
				html += "<span class=\"listYear\">" + metadata.year + this.getWatchedIconHtml(metadata.lastViewedAt, metadata.viewOffset, metadata.viewCount, metadata.duration) + "</div>";
			}
		...
		case "track":				
			html = "<li class=\"" + mediaType + "\"><a data-key-index=\"" + index + "\" data-server-url=\"" + serverUrl + "\" data-server-token=\"" + serverToken + "\" data-title=\"" + title + "\" data-key=\"" + key + "\" data-section-key=\"" + metadata.sectionKey + "\" data-section-type=\"" + sectionType + "\" data-media-type=\"" + mediaType + "\" data-media=\"" + metadata.media + "\" href>";
			html += "<div class=\"track-title\"><i class=\"glyphicon\"></i>" + metadata.index + ". " + title + "</div>";			
			html += "</a></li>";	
			break;

		default:				
			html = "<li class=\"list " + metadata.filter + "\"><a data-key-index=\"" + index + "\" data-server-url=\"" + serverUrl + "\" data-server-token=\"" + serverToken + "\" data-title=\"" + title + "\"";
			html += " data-key=\"" + key + "\" data-section-type=\"" + sectionType + "\" data-section-key=\"" + metadata.sectionKey + "\" data-media-type=\"folder\" data-filter=\"" + metadata.filter + "\" href>";
			html += "<span class=\"title\">" + title + "</div>";			
			html += "</a></li>";	
			break;
...
PLEX.prototype.getSharedMediaPreviewHtml = function(serverUrl, serverToken, xml) {
	var mediaItem = $(xml).find("MediaContainer:first").children().first();
	var mediaItemFile = mediaItem.find("Media:first");	
	var mediaType = mediaItem.attr("type");	
	var audioStream = $(xml).find("Media:first Stream[streamType='2'][selected='1']").length > 0 ? $(xml).find("Media:first Stream[streamType='2'][selected='1']") : $(xml).find("Media:first Stream[streamType='2']:first");
	...
	switch(mediaType) {
		case "movie":
			html += "<div class=\"leftColumn\">";
			html += "<div class=\"cover\" style=\"background-image: url('" + this.getSharedTranscodedPath(serverUrl, serverToken, mediaItem.attr("thumb"), 300, 440) + "')\"></div>";

			html += "<div class=\"media\">";
			if (mediaItemFile.attr("videoResolution")) { html += "<img src=\"" + this.getSharedTranscodedMediaFlagPath(serverUrl, serverToken, "videoResolution", mediaItemFile.attr("videoResolution"), 100, 20) + "\"/>"; } 
			if (mediaItemFile.attr("audioCodec")) { html += "<img src=\"" + this.getSharedTranscodedMediaFlagPath(serverUrl, serverToken, "audioCodec", mediaItemFile.attr("audioCodec"), 100, 20) + "\"/>"; }
			if (mediaItemFile.attr("audioChannels")) { html += "<img src=\"" + this.getSharedTranscodedMediaFlagPath(serverUrl, serverToken, "audioChannels", mediaItemFile.attr("audioChannels"), 100, 20) + "\"/>"; }
			html += "</div>";
			...
			if (mediaItem.attr("studio")) { html += "<div class=\"studio\"><img src=\"" + this.getSharedTranscodedMediaFlagPath(serverUrl, serverToken, "studio", mediaItem.attr("studio"), 300, 40) + "\"/></div>"; }
			if (mediaItem.attr("contentRating")) { html += "<div class=\"contentRating\"><img src=\"" + this.getSharedTranscodedMediaFlagPath(serverUrl, serverToken, "contentRating", mediaItem.attr("contentRating"), 70, 25) + "\"/></div>"; }	
		...
		case "photo":
			...
			html += "<div class=\"rightColumn\">";								
			html += "<div class=\"cover\" style=\"background-image: url('" + this.getSharedTranscodedPath(serverUrl, serverToken, mediaItem.attr("thumb"), 800, 600) + "')\"></div>";
			html += "</div>";
			break;
			
		case "album":
			html += "<div class=\"leftColumn\">";
			html += "<div class=\"cover\" style=\"background-image: url('" + this.getSharedTranscodedPath(serverUrl, serverToken, mediaItem.attr("thumb"), 320, 320) + "')\"></div>";
			html += "<div class=\"title\">" + mediaItem.attr("title") + "</div>";
			html += "<div class=\"time\">";
			if (mediaItem.attr("year")) { html += "<span class=\"year\">" + mediaItem.attr("year") + "</span>"; }
			html += "</div>"; 
		...
		case "artist":
			html += "<div class=\"leftColumn\">";
			html += "<div class=\"cover\" style=\"background-image: url('" + this.getSharedTranscodedPath(serverUrl, serverToken, mediaItem.attr("thumb"), 380, 380) + "')\"></div>";
			html += "<div class=\"title\">" + mediaItem.attr("title") + "</div>";						
			if (mediaItem.find("Genre").length > 0) { html += "<div class=\"genre\">" + this.getAttributeList(mediaItem.find("Genre"), "tag", ", ") + "</div>"; }				
			if (mediaItem.attr("summary")) { html += "<div class=\"summary\">" + mediaItem.attr("summary") + "</div>"; }			
			html += "</div>";
		...
		case "season":		
			...
			html += "<div class=\"rightColumn\">";
			html += "<div id=\"children\"><ul></ul></div>";	
			if (mediaItem.attr("theme")) { html += "<div id=\"theme\" data-src=\"" + serverUrl + mediaItem.attr("theme") + "&X-Plex-Token=" + serverToken + "\"></div>" };											
			html += "</div>";			
			break;

		case "episode":
			html += "<div class=\"leftColumn\">";
			html += "<div class=\"cover\" style=\"background-image: url('" + this.getSharedTranscodedPath(serverUrl, serverToken, mediaItem.attr("thumb"), 380, 256) + "')\"></div>";
			html += "<div class=\"media\">";
			if (mediaItemFile.attr("videoResolution")) { html += "<img src=\"" + this.getSharedTranscodedMediaFlagPath(serverUrl, serverToken, "videoResolution", mediaItemFile.attr("videoResolution"), 100, 20) + "\"/>"; } 
			if (mediaItemFile.attr("audioCodec")) { html += "<img src=\"" + this.getSharedTranscodedMediaFlagPath(serverUrl, serverToken, "audioCodec", mediaItemFile.attr("audioCodec"), 100, 20) + "\"/>"; }
			if (mediaItemFile.attr("audioChannels")) { html += "<img src=\"" + this.getSharedTranscodedMediaFlagPath(serverUrl, serverToken, "audioChannels", mediaItemFile.attr("audioChannels"), 100, 20) + "\"/>"; }
...
PLEX.prototype.getSharedMediaMetadata = function(ServerUrl, ServerToken, key, callback) {
	if (key.indexOf("?") > -1) {
		$.get(ServerUrl + key + "&X-Plex-Access-Time=" + this.time + "&X-Plex-Token=" + ServerToken, callback);
	} else {
		$.get(ServerUrl + key + "?X-Plex-Access-Time=" + this.time + "&X-Plex-Token=" + ServerToken, callback);
	}
};

PLEX.prototype.getSharedMediaHtml = function(serverUrl, serverToken, title, mediaType, metadata) {
	var html = "<div class=\"" + mediaType + "\">";
	var serverPort = this.getSharedServerPort(serverUrl);
	
	switch(mediaType) {	
		case "movie":
			html += "<div class=\"thumb\" style=\"background-image: url('" + this.getSharedTranscodedPath(serverUrl, serverToken, metadata.art, 350, 350) + "')\"></div>";
			html += "<div class=\"title\">" + title + "</div>";
			if (metadata.year) { 
				html += "<div class=\"time\"><span class=\"duration\">" + this.getTimeFromMS(metadata.duration);
				html += this.getWatchedIconHtml(metadata.lastViewedAt, metadata.viewOffset, metadata.viewCount, metadata.duration); 
		...
		case "photo":	
			html = "<div class=\"thumb\" style=\"background-image: url('" + this.getSharedTranscodedPath(serverUrl, serverToken, metadata.thumb, 350, 350) + "')\"></div>";	
			html += "<div class=\"title\">" + title + "</div>";	
			if (metadata.year) { html += "<div class=\"time\">" + metadata.year + "</div>"; }				
			if (metadata.summary) { html += "<div class=\"summary\">" + metadata.summary + "</div>"; }	
			html += "<div class=\"dimensions\">";						
...
PLEX.prototype.getSharedSectionDetails = function(serverUrl, serverToken, key, callback) {
	$.get(serverUrl + "/library/sections/" + key + "?X-Plex-Token=" + serverToken, callback);
};

PLEX.prototype.getSharedSectionMedia = function(serverUrl, serverToken, key, filter, filterKey, callback) {
	var jsonCallback = 'callback' + new Date().getTime();
	...
	if (filterKey) {
		switch(filter) {
			...
			case "all": 
				filter = decodeURIComponent(filterKey);
				if (filter.indexOf("?") > -1) {
					$.get(serverUrl + filter + "&X-Plex-Access-Time=" + this.time + "&X-Plex-Token=" + serverToken, callback);
				} else {
					$.get(serverUrl + filter + "?X-Plex-Access-Time=" + this.time + "&X-Plex-Token=" + serverToken, callback);
				}
				break;
			...
			case "search": 
				$.get(serverUrl + "/search?query=" + filterKey + "&X-Plex-Token=" + serverToken, callback);
				break;
								
			default:
				$.get(serverUrl + "/library/sections/" + key + "/" + filter + "/" + filterKey + "?X-Plex-Access-Time=" + this.time + "&X-Plex-Token=" + serverToken, callback);		
				break;
		}
	} else {
		$.get(serverUrl + "/library/sections/" + key + "/" + filter + "?X-Plex-Access-Time=" + this.time + "&X-Plex-Token=" + serverToken, callback);		
	}
};

PLEX.prototype.reportSharedProgress = function(serverUrl, serverToken, key, state, time) {
	$.get(serverUrl + "/:/progress?key=" + key + "&identifier=com.plexapp.plugins.library&time=" + time + "&state=" + state + "&X-Plex-Token=" + serverToken, null);
};

PLEX.prototype.setSharedWatched = function(serverUrl, serverToken, key, callback) {
	$.get(serverUrl + "/:/scrobble?key=" + key + "&identifier=com.plexapp.plugins.library" + "&X-Plex-Token=" + serverToken, callback);
};

PLEX.prototype.setSharedAudioStream = function(serverUrl, serverToken, partKey, streamKey) {
	$.ajax({
		type: "PUT",
		url: serverUrl + "/library/parts/" + partKey + "?audioStreamID=" + streamKey + "&X-Plex-Token=" + serverToken
	});
};

PLEX.prototype.setSharedSubtitleStream = function(serverUrl, serverToken, partKey, streamKey) {
	$.ajax({
		type: "PUT",
		url: serverUrl + "/library/parts/" + partKey + "?subtitleStreamID=" + streamKey + "&X-Plex-Token=" + serverToken
	});
};

PLEX.prototype.getSharedHlsTranscodeUrl = function(serverUrl, serverToken, key, options) {	
	options = options || {};
