...
PlayerShared.prototype.djabDebug = function(message)
{
	//if ($("#infoBox").is(":visible")) {
		//$("#infoBox").hide();
	//} else {
	var previousContent = $("#infoBox .content").html(); 
		$("#infoBox .content").html(previousContent + "<br/>" + message);
		$("#infoBox").show().delay(5000).fadeOut(10000);
	//}
};

PlayerShared.prototype.setDefaultStreams = function()
{
	try {
		var defaultSubtitles = $(this.cache).find("Stream[streamType='3'][selected='1']");	
		if (defaultSubtitles.length > 0) {
			this.djabDebug("Encontrei legendas default");
			this.enableSubtitles(this.serverUrl, this.serverToken, $(defaultSubtitles).attr("key"));
		}
	}
	catch(err) {
		this.djabDebug(err);
...
PlayerShared.prototype.resumeDialog = function(ms)
{
	try {
		var self = this;
...
PlayerShared.prototype.initControls = function()
{
	try {
		var self = this;
...
PlayerShared.prototype.play = function(speed)
{
	var self = this;
	...
	this.timer = setInterval(function() {
		...
		if (self.progressCount >= 30) {
			self.setWatchedStatus(self.serverUrl, self.serverToken, self.mediaKey, self.media.playTime, self.media.playPosition);
			self.plex.reportSharedProgress(self.serverUrl, self.serverToken, self.mediaKey, "playing", self.media.playPosition);
...
PlayerShared.prototype.rewind = function()
{
	try {
...
PlayerShared.prototype.forward = function()
{
	try {
		var pos = Number(this.media.playPosition);
		var total = Number(this.media.playTime);
...
PlayerShared.prototype.pause = function()
{
	try {
		//$("#play").focus();
		clearInterval(this.timer);
...
PlayerShared.prototype.stop = function()
{
	try {
		//$("#play").focus();
		clearInterval(this.timer);	
...
PlayerShared.prototype.seek = function(timeMS)
{
	try {
...
PlayerShared.prototype.disableSubtitles = function()
{
	try {
...
PlayerShared.prototype.toggleControls = function()
{
	try {
		if ($("#controls").is(':visible')) {
			this.hideControls();
...
PlayerShared.prototype.showControls = function()
{
	try {
		$("#controls").show();
		$(".options").show();
...
PlayerShared.prototype.hideControls = function()
{
	try {
		$("#controls").fadeOut();
		$(".options").fadeOut();
...
PlayerShared.prototype.timerControls = function() {
	try {
		var self = this;
...
PlayerShared.prototype.getLanguageCode = function(code) 
{
	try {
...
PlayerShared.prototype.showLoader = function(message)
{
	try {
...
PlayerShared.prototype.hideLoader = function()
{
	try {
		$("#loader").hide();
	}
...
PlayerShared.prototype.setAudoLanguage = function(key, label)
{
	try {
...
PlayerShared.prototype.initialise = function()
{
	try {
		var self = this;
		this.plex = new PLEX();
		...
		this.media.onPlayStateChange = function(event) {
			...
			switch(state) {
				case 0: //Stopped
					...
					$("#message").show();
					$("#message").fadeOut(3000);
					self.plex.reportSharedProgress(self.serverUrl, self.serverToken, self.mediaKey, "stopped", self.media.playPosition);			
					break;
				...
				case 2: //Paused
					...
					$("#message").show();
					this.plex.reportSharedProgress(self.serverUrl, self.serverToken, self.mediaKey, "paused", self.media.playPosition);			
					break;
				...
				case 5: //Finished
					...
					switch ($(self.cache).find("Video:first").attr("type")) {
						case "episode":	
							...
							self.plex.getSharedMediaMetadata(self.serverUrl, self.serverToken, parentKey + "/children", function(xml) {
...
PlayerShared.prototype.setWatchedStatus = function(serverUrl, serverToken, key, duration, current)
{
	try {
		if (current/duration > 0.9) {
			this.plex.setSharedWatched(serverUrl, serverToken, key, null);
		}
	}
	catch(err) {
		this.djabDebug(err);
...
PlayerShared.prototype.enableSubtitles = function(serverUrl, serverToken, key)
{	
	try {
		if (key != "undefined") {
			...
			if (url.indexOf("?") > -1) {
				url = url + "&X-Plex-Token=" + serverToken;
			} else {
				url = url + "?X-Plex-Token=" + serverToken;
			}
...
PlayerShared.prototype.infoBox = function()
{
	try {
		var self = this;
		...
		if ($("#infoBox").is(":visible")) {
			...
			$("#infoBox .content").html(self.plex.getSharedMediaHtml(self.serverUrl, self.serverToken, metadata.attr("title"), metadata.attr("type"), 
...
PlayerShared.prototype.languageDialog = function()
{
	try {
		var self = this;
		var html = "";
		...
		$("#dialog a").click(function(event) {
			...
			if ($(this).data("languageCode") != "close") {
				self.plex.setSharedAudioStream(self.serverUrl, self.serverToken, $(this).data("partKey"), $(this).data("streamKey"));
...
PlayerShared.prototype.subtitleDialog = function()
{
	try {
		var self = this;
		var html = "";
		...
		$("#dialog a").click(function(event) {
			...
			switch ($(this).data("key")) {
				case "disabled":
					self.plex.setSharedSubtitleStream(self.serverUrl, self.serverToken, $(this).data("partKey"));
				...
				default:
					self.plex.setSharedSubtitleStream(self.serverUrl, self.serverToken, $(this).data("partKey"), $(this).data("streamKey"));
					self.enableSubtitles(self.serverUrl, self.serverToken, $(this).data("key"));
					break;
			}
...
PlayerShared.prototype.openMedia = function(key)
{
	try {
		var self = this;
		this.key = key;
		...
		this.plex.getSharedMediaMetadata(self.serverUrl, self.serverToken, this.key, function(xml) {
			self.cache = xml;
			...
			if (self.directPlay) {
				...
				var url = self.serverUrl + self.mediaUrl;

				if (url.indexOf("?") > -1) {
					url = url + "&X-Plex-Token=" + self.serverToken;
				} else {
					url = url + "?X-Plex-Token=" + self.serverToken;
				}
				...
				self.media.data = self.plex.getSharedHlsTranscodeUrl(self.serverUrl, self.serverToken, self.key);			
			}
			self.hideLoader();
			
			//self.djabDebug(self.media.data);
